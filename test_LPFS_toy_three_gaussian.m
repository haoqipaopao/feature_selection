...
hl=figure('name','Projected directions by LPFS, PCA, LPP, PCAN and LDA'); 
set(gca, 'fontsize',15);
%plot(X(:,1),X(:,2),'.k'); hold on;
...
if abs(W(1))>abs(W(2))
    h4 = plot([minx, maxx],[W(2)/W(1)*minx, W(2)/W(1)*maxx],'b','LineWidth',2,'MarkerSize',15); hold on;
else
    h4 = plot([W(1)/W(2)*miny, W(1)/W(2)*maxy], [miny, maxy],'b','LineWidth',2,'MarkerSize',15); hold on;
end;

axis equal;
legend([h0,h2,h3,h4],'RLDA','PCA','LPP','LDA',4);
saveas(hl,[savePath '\comp' num2str(idx) '-' num2str(m) '-prj-3-' num2str(datatype) '.eps'],'psc2');
saveas(hl,[savePath '\comp' num2str(idx) '-' num2str(m) '-prj-3-' num2str(datatype) '.jpg']);
